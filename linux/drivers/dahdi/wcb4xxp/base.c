...
#ifdef BX00P_OPEC_HW
extern int OPVXXP_INIT(u_int ,OPVXEC_HW *,BYTE);
extern void OPVXXP_EXIT( OPVXEC_HW*);
#endif
...
#ifdef BX00P_OPEC_HW
static void opv_echocan_free(struct dahdi_chan *chan, struct dahdi_echocan_state *ec);
#else
...
#ifdef BX00P_OPEC_HW
static void hfc_assign_bchan_fifo_opvxec(struct b4xxp *b4, int port, int bchan){
	int fifo, hfc_chan;
	unsigned long irq_flags;
	static int first=1;
 ...
 */
	/* S/T -> PCM */	//tx direciton
	hfc_setreg_waitbusy(b4, R_FIFO, ((fifo+16) << 1));	//transmit  FIFO data  + low 16  FIFO number used for transmit
	b4xxp_setreg8(b4, A_CON_HDLC, 0xc0 | V_HDLC_TRP | V_IFF);	// S/T -> PCM 
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT));
	b4xxp_setreg8(b4, R_SLOT, (((hfc_chan/4)*8) + ((hfc_chan%4)*4) + 1) << 1);	// transmit PCM data  +  PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | (hfc_chan << 1));	//HFC-channel for transmit data + low 16 HFC-channel number  +   STIO1 output
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------S/T -> PCM,tx direciton fifo=%d,chan=%d,slot = %d----------\n",fifo+16,hfc_chan,((hfc_chan/4)*8) + ((hfc_chan%4)*4) + 1);
	
	/* PCM -> FIFO */	//rx direction
	hfc_setreg_waitbusy(b4, R_FIFO,(fifo << 1) | 1);	//0x20 | receive FIFO data +  high 16 FIFO number  used for receive
	b4xxp_setreg8(b4, A_CON_HDLC, 0x20 | V_HDLC_TRP | V_IFF);	// PCM->FIFO
	b4xxp_setreg8(b4, A_CHANNEL, ((16+hfc_chan) << V_CH_FNUM_SHIFT) | V_CH_FDIR);//
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0);
	b4xxp_setreg8(b4, A_IRQ_MSK, 0);
	b4xxp_setreg8(b4, R_SLOT, (((((hfc_chan)/4)*8) + (((hfc_chan)%4)*4) + 1) << 1) | 1);//receive PCM data  + PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x20 | ((16+hfc_chan) << 1) | 1);	//HFC-channel for receive data + high 16 HFC-channel number  +   receive from STIO2
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------PCM -> FIFO rx direction fifo=%d,chan=%d, PCM -> FIFO slot = %d----------\n",fifo,hfc_chan,(((hfc_chan)/4)*8) + (((hfc_chan)%4)*4) + 1);
	
		/* FIFO -> PCM */	//tx direction
	hfc_setreg_waitbusy(b4, R_FIFO, (fifo << 1));		//0x20 | transmit FIFO data + high 16 FIFO number used for transmit
	b4xxp_setreg8(b4, A_CON_HDLC, 0x20 | V_HDLC_TRP | V_IFF);	//FIFO->PCM
	b4xxp_setreg8(b4, A_CHANNEL, ((hfc_chan+16) << V_CH_FNUM_SHIFT));//
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0);
	b4xxp_setreg8(b4, A_IRQ_MSK, 0);
	b4xxp_setreg8(b4, R_SLOT, ((((hfc_chan)/4)*8) + (((hfc_chan)%4)*4)) << 1);//transmit PCM data  +  PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x20 | ( (hfc_chan+16)<< 1));	//HFC-channel for receive data + high 16 HFC-channel number  +  output STIO1
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------FIFO -> PCM tx direction fifo=%d,chan=%d,FIFO -> PCM slot = %d----------\n",fifo,hfc_chan,(((hfc_chan)/4)*8) + (((hfc_chan)%4)*4));
	
	/* PCM -> S/T */	//rx direction
	hfc_setreg_waitbusy(b4, R_FIFO, ((fifo+16) << 1) | 1);
	b4xxp_setreg8(b4, A_CON_HDLC, 0xc0 | V_HDLC_TRP | V_IFF);	// PCM -> S/T
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT) | V_CH_FDIR);
	b4xxp_setreg8(b4, R_SLOT, ((((hfc_chan/4)*8) + ((hfc_chan%4)*4)) << 1) | 1);//receive PCM data  + PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x40 | (hfc_chan << 1) | 1);	//HFC-channel for receive data + low 16 HFC-channel number  +  rx STIO1
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------PCM -> S/T rx direction fifo=%d,chan=%d,PCM -> S/T slot = %d----------\n",fifo+16,hfc_chan,(hfc_chan/4)*8 + ((hfc_chan%4)*4));
	
	flush_pci();
	spin_unlock_irqrestore(&b4->fifolock, irq_flags);
}

static void hfc_assign_bchan_fifo_opvxec2(struct b4xxp *b4, int port, int bchan){
	int fifo, hfc_chan;
	unsigned long irq_flags;
	static int first=1;
 ...
 */
	...
	b4xxp_setreg8(b4, A_CON_HDLC, 0xc0 | V_HDLC_TRP | V_IFF);	// S/T -> PCM 
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT));
	b4xxp_setreg8(b4, R_SLOT, (((hfc_chan/4)*8) + ((hfc_chan%4)*4) + 1 + 16) << 1);	// transmit PCM data  +  PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | (hfc_chan << 1));	//HFC-channel for transmit data + low 16 HFC-channel number  +   STIO1 output
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------S/T -> PCM,tx direciton fifo=%d,chan=%d,slot = %d----------\n",fifo+16,hfc_chan,((hfc_chan/4)*8) + ((hfc_chan%4)*4) + 1+16);
	...
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0);
	b4xxp_setreg8(b4, A_IRQ_MSK, 0);
	b4xxp_setreg8(b4, R_SLOT, (((((hfc_chan)/4)*8) + (((hfc_chan)%4)*4) + 1+16) << 1) | 1);//receive PCM data  + PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x20 | ((16+hfc_chan) << 1) | 1);	//HFC-channel for receive data + high 16 HFC-channel number  +   receive from STIO2
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------PCM -> FIFO rx direction fifo=%d,chan=%d, PCM -> FIFO slot = %d----------\n",fifo,hfc_chan,(((hfc_chan)/4)*8) + (((hfc_chan)%4)*4) + 1+16);
	...
	b4xxp_setreg8(b4, A_SUBCH_CFG, 0);
	b4xxp_setreg8(b4, A_IRQ_MSK, 0);
	b4xxp_setreg8(b4, R_SLOT, ((((hfc_chan)/4)*8) + (((hfc_chan)%4)*4)+16) << 1);//transmit PCM data  +  PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x20 | ( (hfc_chan+16)<< 1));	//HFC-channel for receive data + high 16 HFC-channel number  +  output STIO1
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------FIFO -> PCM tx direction fifo=%d,chan=%d,FIFO -> PCM slot = %d----------\n",fifo,hfc_chan,(((hfc_chan)/4)*8) + (((hfc_chan)%4)*4)+16);
	
	/* PCM -> S/T */	//rx direction
	hfc_setreg_waitbusy(b4, R_FIFO, ((fifo+16) << 1) | 1);
	b4xxp_setreg8(b4, A_CON_HDLC, 0xc0 | V_HDLC_TRP | V_IFF);	// PCM -> S/T
	b4xxp_setreg8(b4, A_CHANNEL, (hfc_chan << V_CH_FNUM_SHIFT) | V_CH_FDIR);
	b4xxp_setreg8(b4, R_SLOT, ((((hfc_chan/4)*8) + ((hfc_chan%4)*4)+16) << 1) | 1);//receive PCM data  + PCM  time slot number
	b4xxp_setreg8(b4, A_SL_CFG, 0x80 | 0x40 | (hfc_chan << 1) | 1);	//HFC-channel for receive data + low 16 HFC-channel number  +  rx STIO1
	hfc_setreg_waitbusy(b4, A_INC_RES_FIFO, V_RES_FIFO);
	//printk("---------PCM -> S/T rx direction fifo=%d,chan=%d,PCM -> S/T slot = %d----------\n",fifo+16,hfc_chan,(hfc_chan/4)*8 + ((hfc_chan%4)*4)+16);
...
static void hfc_init_all_st(struct b4xxp *b4)
{
    ...
    for (i=0; i < b4->numspans; i++) {
        ...
        {
#ifdef BX00P_OPEC_HW
            if((vpmsupport)&&((b4->card_type == B400P_OV)||(b4->card_type == B200P_OV)))
                nt = ((gpio & (1 << (i + 4))) != 0);
            else
#endif
                if (IS_B410P(b4)) {
                    ...
                    nt = ((gpio & (1 << (i + 4))) != 0);
                    dev_info(&b4->pdev->dev, "Port %d: %s mode\n", i + 1, (nt ? "NT" : "TE"));			
                }
        }
        else
        {
            nt = !((te_nt_override >> i) & 1);
            dev_info(&b4->pdev->dev, "Port %d: %s mode manually set\n", i + 1, (nt ? "NT" : "TE"));		    
        }
        s->te_mode = !nt;
...
#ifdef BX00P_OPEC_HW
	if((vpmsupport)&&((b4->card_type == B400P_OV)||(b4->card_type == B200P_OV)))
		b4xxp_setreg8(b4, R_BRG_PCM_CFG,0x02 | V_PCM_CLK);
	else
#endif
...
#ifdef BX00P_OPEC_HW
static void b4xxp_init_opvxec(struct b4xxp *b4){
	int ecRet;
	unsigned long flag;
	
	if((vpmsupport)&&(b4->card_type == B400P_OV)){		
		b4xxp_setreg8(b4, R_GPIO_SEL,0x70 );
		b4xxp_setreg8(b4, R_GPIO_EN1,0x10 );
		
		b4xxp_setreg8(b4, R_GPIO_OUT1, 0x10);
		udelay(1000);
/**/
		b4->ecflag = 0;
		ecRet=OPV_ERROR;
		...
		local_irq_save(flag);
		ecRet=OPVXXP_INIT(b4->pci_iobase,&b4->opvhw,CS_1);
		local_irq_restore(flag);
		if(ecRet == OPV_SUCCESS){
			local_irq_save(flag);
			OPVXXP_INIT(b4->pci_iobase,&b4->opvhw,CS_2);
			local_irq_restore(flag);
		}
		if(ecRet == OPV_SUCCESS)
			b4->ecflag = 1;
	...
	if((vpmsupport)&&(b4->card_type == B200P_OV)){		
		b4xxp_setreg8(b4, R_GPIO_SEL,0x70 );
		b4xxp_setreg8(b4, R_GPIO_EN1,0x10 );
...
/**/
		...
		local_irq_save(flag);
		ecRet=OPVXXP_INIT(b4->pci_iobase,&b4->opvhw,CS_2);
		local_irq_restore(flag);
		if(ecRet == OPV_SUCCESS)
			b4->ecflag = 1;
	}
 ...
 */
 ...
 #ifdef BX00P_OPEC_HW
            if((vpmsupport)&&(b4->card_type == B400P_OV) &&(b4->ecflag == 1)){
				hfc_assign_bchan_fifo_opvxec(b4, span, 0);
				hfc_assign_bchan_fifo_opvxec(b4, span, 1);
			}	
			else if((vpmsupport)&&(b4->card_type == B200P_OV)&&(b4->ecflag == 1)&&(span < 2)){
				hfc_assign_bchan_fifo_opvxec2(b4, span, 0);
				hfc_assign_bchan_fifo_opvxec2(b4, span, 1);
			}
			else	
#endif	 		
...
static void b4xxp_update_leds_hfc(struct b4xxp *b4)
{
	...
	leds |= green_leds;
	b4xxp_setreg8(b4, R_GPIO_EN1, leds  | 0x10);	/*OPVX*/
	hfc_gpio_set(b4, green_leds  | 0x10);	 		/*OPVX*/
...
#ifdef BX00P_OPEC_HW
static int opv_echocan_create(struct dahdi_chan *chan, struct dahdi_echocanparams *ecp,
...
#ifdef BX00P_OPEC_HW
	.echocan_create = opv_echocan_create,
#else
...
#ifdef BX00P_OPEC_HW
	b4->pci_iobase = (u_int) pci_resource_start(b4->pdev, 0);
#endif
