...
static void get_systime_val(int *mon, int *mday, int *hour, int *min)
{
	...
	seconds = ktime_to_ms(now) / 1000;   

#define SECS_PER_HOUR	(60 * 60)
#define SECS_PER_DAY	(SECS_PER_HOUR * 24)
    days = seconds / SECS_PER_DAY;
    rem = seconds % SECS_PER_DAY;
...
#else
static struct proc_ops proc_param_off_fops = {
    .proc_open = open_param_off_proc, 
    .proc_read = seq_read,
    .proc_write = write_param_off_proc,
    .proc_lseek = seq_lseek,
    .proc_release = single_release,
}; 
#endif
