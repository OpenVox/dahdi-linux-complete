...
static volatile int kwel_time = 500;
...
static volatile int maxpulsetime = DAHDI_MAXPULSETIME/8 ;  // ms
 ...
 *points to channel's data structure (struct wctdm or struct a24xx e.g)
 */
...
static int dahdi_hangup(struct dahdi_chan *chan)
{
	...
	if (chan->span->flags & DAHDI_FLAG_RBS) {
		if (chan->sig == DAHDI_SIG_CAS) {
			...
			dahdi_rbs_sethook(chan, DAHDI_TXSIG_KEWL, DAHDI_TXSTATE_KEWL, kwel_time);
		} else
...
static int initialize_channel(struct dahdi_chan *chan)
{
	...
	//chan->rxminflashtime = DAHDI_DEFAULT_RXMINFLASHTIME;
	chan->rxflashtime = DAHDI_DEFAULT_RXMAXFLASHTIME;
...
static void __do_dtmf(struct dahdi_chan *chan)
{
    ...
    while ((c = chan->txdialbuf[0])) {
        /* Add 'W' before CID to delay the sending of CID, if sending CID as soon as FXO off hook, 
         * not waiting for FXS dial tone, FXS may fail to get CID because FXS is not ready,
         * thus FXO can't call out, so add the delay between FXO off hook and start sendding CID */
        if((('T' == c) ||('M' == c) ||('O' == c) ||('R' == c) ||('P' == c)) && (w_count >0 )) {
            memmove(chan->txdialbuf + 1 + w_count, chan->txdialbuf + 1, sizeof(chan->txdialbuf) - 1 - w_count);
            for(i = 0; i < w_count; i++) {
                chan->txdialbuf[1 + i] = 'W';
            }
        }
...
static inline void __rbs_otimer_expire(struct dahdi_chan *chan)
{
	...
	case DAHDI_TXSTATE_KEWL:
		dahdi_rbs_sethook(chan, DAHDI_TXSIG_ONHOOK, DAHDI_TXSTATE_AFTERKEWL, kwel_time);
		if (chan->file && (chan->file->f_flags & O_NONBLOCK))
			__qevent(chan, DAHDI_EVENT_HOOKCOMPLETE);
		wake_up_interruptible(&chan->waitq);
		break;
	   ...
	   case DAHDI_SIG_FXOKS: /* FXO Kewlstart */
		switch(rxsig) {
			...
			if (chan->itimer) /* if timer still running */
			{
			    int plen = chan->itimerset - chan->itimer;
				if(debug)
					printk(KERN_DEBUG "plen =%d , maxpulsetime*8 = %d\n",plen,maxpulsetime*8);
...
module_param(maxpulsetime, int, 0644);
...
module_param(kwel_time,int, 0600);
