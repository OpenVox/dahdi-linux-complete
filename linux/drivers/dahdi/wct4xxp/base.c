...
static void __t4_set_timing_source_auto(struct t4 *wc)
{
	...
	firstprio = secondprio = wc->numspans;
	...
	if (timingcable) {
		...
		if (firstprio != wc->numspans) {
			...
			dev_info(&wc->dev->dev, "Recovered timing mode, "\
						"RCLK set to span %d\n",
						firstprio+1);
		} else if (secondprio != wc->numspans) {
			wc->syncsrc = -1;
		...
		for (i = 0; i < wc->numspans; i++) {
            ...
            if(-1 == wc->syncsrc) 
                wc->tspans[i]->span.syncsrc = 0;
            else {
                /* Actually wc->syncsre is span.offset, so we need to convert span.offset to span.spanno */
                wc->tspans[i]->span.syncsrc = wc->tspans[wc->syncsrc]->span.spanno;
                /* 1 <= span.syncsrc <= wc->numspans */
                wc->tspans[i]->span.syncsrc = wc->tspans[i]->span.syncsrc % wc->numspans ?
                                              wc->tspans[i]->span.syncsrc % wc->numspans : wc->numspans;
            }
